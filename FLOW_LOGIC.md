...
**Support-side signing CLI:** `scripts/make-fix-code.py` generates paste-ready envelopes: `python scripts/make-fix-code.py session_idle_minutes 60 | clip`. It resolves `FIX_CODE_SECRET` from `--secret` → `$FIX_CODE_SECRET` → `backend_fastapi/.env`, casts values to JSON types before signing ("true"→true, "60"→60; leading-zero digit runs stay strings — pass `--string` to sign verbatim), prints only the JSON envelope to stdout (diagnostics to stderr), and exits 1 without a secret. `backend_fastapi/tests/test_make_fix_code_cli.py` (36 tests) enforces byte-parity between the CLI's `sign_payload` and the backend route's — change the two together or every code will 403.
