...
## [SPEC 09 FOLLOW-UP — 2026-09-25] Fix-Code CLI + Contract Parity — COMPLETE
### Remote-fix tooling
- scripts/make-fix-code.py: NEW — operator CLI that mints the HMAC envelope for `POST /api/v1/support/fix-code/verify`. Resolves `FIX_CODE_SECRET` automatically (--secret > env > backend_fastapi/.env via a dependency-free parser), type-casts CLI values (bool/int/float/JSON/string with `--string` override), and prints the exact compact envelope to stdout for piping to the clipboard. Fail-fast (exit 1) when no secret; secret never printed.
- backend_fastapi/.env.example: documents FIX_CODE_SECRET (distinct from SECRET_KEY; empty ⇒ fail-closed).
- backend_fastapi/tests/test_make_fix_code.py: NEW (20 tests) — cross-checks the CLI HMAC against `support_fix_route.sign_payload`, round-trips bool typing through JSON, verifies secret resolution/fail-fast, and POSTs a generated envelope through the real route (applied 200 + tampered 403).
