...
- `src-tauri/sign.cmd` + `tauri.conf.json → bundle.windows.signCommand: "cmd /c src-tauri/sign.cmd %1"`. The script reads the cert from **environment variables only** (never committed): `SIGN_CERT_PATH` (.pfx/.p12 path), `SIGN_CERT_PASSWORD`, optional `SIGN_TIMESTAMP_URL` (default `http://timestamp.digicert.com`), and `TAURI_WINDOWS_SIGNTOOL_PATH` (optional explicit `signtool.exe`). `signtool` is auto-located via `where signtool` or the Windows SDK `bin\x64` path.
