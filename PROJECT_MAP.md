...
- **Tauri release readiness — DONE (2026-08-21):** `tauri.conf.json` now uses `identifier: "com.pharmacy.suite"`, `version: "1.0.0"`, a restrictive `security.csp`, `frontendDist: "../.next/static"`, `bundle.resources` mapping the Next **standalone** server into `resources/standalone`, `externalBin: ["binaries/node"]` (bundled `node.exe` 87 MB sidecar for both `x86_64-pc-windows-msvc` + `x86_64-pc-windows-gnu`), and the main window `url: "http://127.0.0.1:3000"`. The `force-dynamic` export approach was abandoned: the frontend ships 6 Next Route Handlers (`app/api/{health,currency,webhooks/paddle,auth/login,auth/refresh,auth/logout}`) implementing an httpOnly-cookie BFF, so a static `output:"export"` cannot work — the standalone server runs as a Tauri **sidecar** instead. `lib.rs` registers `tauri-plugin-shell` and spawns the node sidecar (`server.js` on `:3000`, robust path resolution over resource-dir candidates) plus a best-effort `python -m uvicorn app.main:app` backend on `:8000`. Capabilities grant `shell:allow-execute/spawn/stdin-write`. Signed installer not yet produced (code-signing cert still required).
