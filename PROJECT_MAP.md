...
| M100 | PriceCode Pricing Bug Fix — `dispense_service.py` lines 201, 238, 247 changed from `payload.price_at_time` (raw input) to `unit` (PriceCode-computed price). Ensures `Dispense.price_at_time`, `ReceiptItem.price_at_time`, and `SoldItem.price` all use the computed PriceCode unit price. 12 new coverage tests in `tests/test_dispense.py` (price code markup/cost_factor/fee-only/min-clamp/max-clamp, insurance inactive/valid/missing, lot AWP pricing, refill success/exhausted/not-found, product not found, no-price-code fallback). Backend: 630 passed, 92.24% coverage. | Verified | 2026-09-01 |
| M101 | Full Frontend i18n — All 16 Pages Wired — `en.json` expanded 13 → 490 keys across 16 namespaces (login, common, nav, pos, inventory, patients, analytics, dashboard, settings, users, roles, audit, email, backup, prescribers, wcClaims, license). All 6 locale files (`en`, `de`, `es`, `fr`, `pt`, `ar`) updated with matching keys. 16 pages fully wired to `useI18n()` / `t()`: login (6), DashboardNav (13), POS (38), inventory (127), patients (101), analytics demand (22), dashboard main (11), settings (8), users (47), roles (22), audit (9), email (14), backup (11), prescribers (36), WC claims (28), license (14) — ~507 strings total. TypeScript compiles clean (`tsc --noEmit` 0 errors). vitest 33/33 pass. | Verified | 2026-09-01 |
...
| **Phase 4: DUR Checks** — Drug Utilization Review (interaction checking, duplicate therapy alerts) | ✅ Complete 2026-09-01 |
| **Phase 5: Email Reporting** — SMTP-based daily sales email using config fields | ✅ Complete 2026-09-01 |
...
| M-FE1 | **FastAPI Backend Phase 1 — BestRx Gap Closure (Backend):** Prescriber CRUD (CRUD + NPI conflict), Rx numbering (`RX-YYYYMMDD-XXXX`), refill tracking (refill_count, refills_authorized, last_fill_date), drug file enrichment (NDC, form, strength, manufacturer_name, therapeutic_class, is_generic, is_controlled, maintenance_medication, drug_cost), patient field enrichment (emergency_contact, Workers' Comp, delivery_status, consent_flag, demographics, communication_preferences), insurance enrichment (plan_type, help_desk_phone, copay_tier, copay_amount, coverage_percentage), price_code enrichment (awp, mac, wac, third_party_pricing). Schema v9 migration. 478 tests pass, 90.04% coverage. `prescriber_route.py`, `lib/api/prescribers.ts`, `app/prescribers/page.tsx` created. `dispense_service.py` refill endpoint added. | ✅ Complete 2026-09-01 |
| M-FE2 | **FastAPI Phase 1 — Frontend Integration (Drug File + Patient Enrichment):** Extended `ProductBase`/`MedicineUpdate` schemas (schemas.py) with 16 drug file enrichment fields. Extended `PatientBase`/`PatientCreate`/`PatientRead`/`PatientUpdate` (schemas.py) with 30+ enrichment fields (emergency contact, employment, WC, demographics, comms, delivery). Extended `Medicine`/`MedicineCreate`/`MedicineUpdate` (contracts.ts) and `PatientBase`/`PatientRead`/`PatientUpdate` (contracts.ts) to mirror backend. Added "Drug Information" tab to inventory page (inline edit table for NDC, form, strength, manufacturer, therapeutic class, generic/controlled/maintenance flags, drug cost, default SIG/qty/days supply, lot #, package, unit). Added 5 new patient tabs: Emergency Contact, Employment, Workers' Comp, Demographics, Communication Preferences (consent flag, prefer_call/text/email toggles, 340B, delivery zone). Fixed `lib/api/prescribers.ts` named import. TypeScript compiles clean, 130+ tests pass with no regression. | ✅ Complete 2026-09-01 |
| M-FE3 | **Phase 2 — Workers' Compensation Full Workflow:** `WorkersCompClaim` ORM model (18 fields: patient_id FK, claim_number unique, carrier info, injury details, employer info, status, dispense_id FK, financials, notes, timestamps). Schema v10 migration with `workers_comp_claims` table + indexes on patient_id and status. `WCClaimRepository` (list/get/get_by_claim_number/create/update/delete). CRUD routes at `/api/v1/wc-claims` (GET list, GET by id, POST create with NPI conflict, PUT update, DELETE). `WCClaimBase`/`WCClaimCreate`/`WCClaimRead`/`WCClaimUpdate` Pydantic schemas. TypeScript `WCClaimBase`/`WCClaimCreate`/`WCClaimRead`/`WCClaimUpdate` interfaces in `contracts.ts`. `lib/api/wc.ts` API client. `app/dashboard/wc-claims/page.tsx` full CRUD page with status filter, color-coded status badges, create/edit modal. DashboardNav link added. Migration version assertion updated (9→10). TypeScript compiles clean, 13 tests pass. | ✅ Complete 2026-09-01 |
| M-FE3b | **Analytics + Config Enhancements:** Added `GET /api/v1/analytics/top-selling` endpoint (aggregates POS `receipt_items` + clinical `dispenses` by product_name, date window filter, ranked by total quantity, configurable limit). `TopSellingItem`/`TopSellingResponse` schemas added to schemas.py. Added SMTP config fields to `Settings` (smtp_host, smtp_port, smtp_user, smtp_password, smtp_from_email, smtp_from_name, smtp_use_tls) for Phase 4 email reporting. | ✅ Complete 2026-09-01 |
...
- `backend_fastapi: python -m pytest -q` → **128 passed** (Phase A/B suite). Green (coverage 91.07%, gate `fail_under=90`).
- `backend_fastapi: python -m mypy app` → **Success: no issues found in 33 source files**.
- Root: `npx tsc --noEmit` → **0 errors** (frontend libs + UI, strict).
- Root: `npx next build` → **Compiled successfully (12/12 pages, exit 0)**. Two non-fatal build-time warnings (`location is not defined`) originate from Next.js 16.2.10's *own* bundled RSC code (`node_modules_next_dist_*`) referencing the browser global `location` during RSC data collection — **not from our code** (zero `location` references in `app/`/`lib/`/`stores/`/`components/`/`hooks/`). R1 mitigations applied: `/` and `/pos` are `force-dynamic` (no static prerender) and `PricingCard` performs checkout via a client-side API call to the Creem BFF (`POST /api/v1/license/checkout`) with no in-browser payment SDK script injection. The warning is a framework prerelease artifact and does not affect the running kiosk (browser provides `location`).
...
### Phase A — POS Operational Security Addendum (DONE, completed 2026-08-17)

Status of the five operational-security concerns (verified against the addendum):

| Concern | State | Notes |
|---|---|---|
| A1 — shift-close variance + auto-approve thresholds | **DONE** | Conditional approval: `cash_tender`/`float_add` auto-approve; `cash_drop ≥ 400` or `paid_out > 50` (or `pickup`/unknown incl. legacy `cash-in`) require a manager token (`X-Approval-Token`, scope `drawer.move`) else 403. Backend: `Shift` model + v4 migration (`SCHEMA_VERSION=4`), `ShiftOpenRequest`/`ShiftRead`/`ShiftCloseRequest`/`ShiftCloseResult`/`ShiftPreviewResult` schemas, `PosService.open_shift`/`close_shift`/`preview_shift` (expected = opening_float + Σcash sales + Σ{float_add,cash_drop,paid_out,pickup}), `POST /shift/open`, `POST /shift/close`, `GET /shift/{id}/preview`. Frontend: `lib/api/shift.ts`, `types/contracts.ts` shift types, `stores/posStore.ts` `shiftId` + `ensureShift`, `components/ShiftCloseDialog.tsx` counted-cash input + expected/variance display + discrepancy manager-PIN gate (`shift.close.variance` when `|variance|>2.00` or negative), `app/pos/page.tsx` auto-opens a shift on mount. Verified by `tests/test_pos_hardening.py::test_shift_close_variance_zero` (T23) + `test_shift_close_variance_computed` + `test_paid_out_requires_approval_over_threshold` (T24); full suite 110 passed; `mypy app --strict` 0; `tsc` 0; `vitest` 23; `next build` 12/12. |
| A2 — offline manager-PIN fallback | **DONE** | `lib/offlineCrypto.ts` (`provisionManagerPolicy`, `verifyPinOffline`, `constantTimeEqual`, `ManagerPolicy`, `OFFLINE_MAX_ATTEMPTS=3`); `lib/db.ts` `STORE_MANAGER_POLICIES` (DB v2); `components/ManagerApprovalDialog.tsx` self-provisions an offline policy on successful online approval and falls back to `verifyPinOffline` only on network errors (never on 401 wrong-PIN), self-wiping after 3 failures. Tests **H19** (constant-time compare) + **T25** (brute-force resistance + self-wipe) in `lib/offlineCrypto.test.ts` (6 tests). Design note: server `pin_hash` is device-bound peppered (C.4) and unrecoverable off-machine, so the client self-derives its own `PBKDF2(pin, salt, 200k)` policy — no backend change required. |
| A3 — cart recovery prompt + 4h GC | **DONE** | `lib/storagePersist.ts` `RECOVERY_WINDOW_MS=4h` + `getCartRegistry`/`registerCart`/`loadCart`/`deleteCart`/`sweepStaleTabs`/`findRecoverable`. `stores/posStore.ts` persists the active cart on every line mutation (add/update/remove/clear/checkout) via `persistLines`, sweeps stale carts in `hydrate`, and exposes `recoverable` + `recoverCart`/`discardRecoverable`. `app/pos/page.tsx` shows a non-blocking recovery banner with Recover/Discard. Verified by `stores/recovery.test.ts` (5 tests: H20×2, H21×2, H22). `tsc`/`vitest` green. |
| A4 — offline-replay 410 → discrepancy | **DONE** | Discrepancy recording + surfacing complete. Backend: `SyncService.push` records `OVER_SOLD_CROSS_TERMINAL` via `SyncRepository.insert_discrepancy`; new `GET /api/v1/sync/discrepancies` (`inventory.read`) + `POST /api/v1/sync/discrepancies/{id}/resolve` (`inventory.write`); `DiscrepancyRead` schema + repo/service methods. Frontend: `types/contracts.ts` `DiscrepancyRead`, `lib/api/sync.ts` `getDiscrepancies`/`resolveDiscrepancy`, `components/DiscrepanciesPanel.tsx` "Synced discrepancies" section with a permission-gated Resolve action. Verified by `tests/test_pos_hardening.py::test_over_sell_discrepancy_is_surfaced`; `tsc`/`next build` green. (Approach differs from the plan's `SyncPushResult.discrepancies`/`markDiscrepant` sketch — the hub already persists discrepancies, so a dedicated fetch/resolve surface is used.) |
| A5 — sync-lock fallback | **DONE (verify only)** | `lib/syncLock.ts` 3-tier (in-memory + BroadcastChannel + `navigator.locks`) avoids the `navigator.locks` TypeError; functionally complete. |

Phase A gate (per approved plan): `pytest tests/test_pos_hardening.py` (T23–T24, A4 surface) + `vitest run lib/offlineCrypto.test.ts stores/posStore.test.ts stores/recovery.test.ts` (H19–H22) green; `tsc --noEmit` 0; `next build` green; `mypy app --strict` 0. All green — Phase A complete.

### Phase B — Hardening / Consolidation / Gaps (B1, B2, B3, B4, B5, B6–B8 DONE; C1–C3 deferred)

| Concern | State | Notes |
|---|---|---|
| B1 — verify | **DONE** | Full gate matrix green: backend `pytest` **110 passed**, `mypy app --strict` 0; frontend `tsc` 0, `vitest` 23, `next build` 12/12. |
| B3 — consolidate `archive/` | **DONE** | Confirmed **zero** active imports of `archive/` from app code. Pruned generated environments/caches (`archive/venv`, `build_venv`, `build`, `dist`, `.vercel`, `__pycache__`). One generated dir `archive/.venv` could not be removed (its `python.exe` is locked by a running process) — regenerable, gitignored, non-authoritative. |
| B5 — fill carried-forward gaps | **DONE** | **Returns/Refunds:** `Refund` model + v5 migration (`SCHEMA_VERSION=5`), `POST /pos/refund` (perm `pos.checkout`), `PosService.refund` (FEFO restock via `InventoryService.return_stock`, immutable, one-per-receipt 409 on re-refund) + negative ledger `Receipt`. **Sales report:** `GET /pos/reports/sales` (`SalesReport`: count/gross/refund_total/net/by_payment_method, perm `inventory.reports`). **Audit immutability:** hash-chain columns on `audit_logs` (`prev_hash`/`entry_hash`); `AuditRepository.log` chains via `system_settings` head; `verify_chain()` + `GET /api/v1/audit/verify` (perm `inventory.read`); tamper test flips `valid=False`. **PHI:** added `phi_encrypt`/`phi_decrypt` (machine-bound DPAPI) + round-trip test; no free-text PHI columns are stored today (only `patient_id` references), so the capability is verified-ready rather than applied. Tests: `tests/test_b5_gaps.py` (3) + `tests/test_phi.py` (2); backend total **110 passed**; `mypy app --strict` 0 (33 files). **Frontend UI:** `RefundDialog` + `SalesReportModal` wired into `app/pos/page.tsx`, permission-gated via the centralized `useCan` hook (`pos.checkout` for refund, `inventory.reports` for the report); `lib/api/pos.ts` `refundSale`/`getSalesReport`; `types/contracts.ts` `RefundRequest`/`RefundRead`/`SalesReport`. Frontend gates green (`tsc` 0, `vitest` 23, `next build` 12/12). |
| B6 — PostgreSQL support | **DONE** (code-complete; live-server verification pending env) | Backend is now **dialect-agnostic**. `app/core/database.py`: `build_engine` only attaches SQLite PRAGMA listeners + `check_same_thread` for `sqlite://` URLs, otherwise builds a server-RDBMS engine (e.g. `postgresql+asyncpg://user:pass@host:5432/db`); `_write_db_path` returns `None` for non-SQLite so the read-replica and `VACUUM INTO` snapshot safely no-op on Postgres (main.py already guards `if dest:`); `create_schema` runs the PRAGMA `migrate_schema` only on SQLite and relies on `Base.metadata.create_all` for Postgres (fresh DB created at latest v6 schema). `config.database_url` is env-driven via `PHARMACY_DB_URL` (still defaults to SQLite for local/dev). Added `asyncpg>=0.29` dependency. **Verification:** no live Postgres exists in this environment, but `tests/test_postgres_ddl.py` compiles every ORM model to PostgreSQL DDL offline (no DB) — proves dialect-agnosticism end-to-end. Backend suite **112 passed** (incl. new DDL test); `mypy app --strict` 0 (33 files). A live Postgres instance is still required for a true integration run and for any future ALTER migrations (recommend Alembic for evolving an existing Postgres schema). |
| B7 — Docker / Nginx / CI | **DONE** (Dockerfiles + compose + nginx + CI authored; image builds verified by CI `docker-build` job — Docker not installed locally to run a live build) | Reproducible deployment for the FastAPI + Next.js stack. **Backend image** `backend_fastapi/Dockerfile`: `python:3.12-slim`, `pip install -e .` (pyproject `[build-system]` added so the editable install works; dry-run confirms all deps resolve), runs `uvicorn app.main:app` on :8000. **Frontend image** `Dockerfile` (root): multi-stage `node:22-slim`, `npm ci` + `next build` (standalone output), runtime copies `.next/standalone` + `.next/static` + `public` and runs `node server.js` on :3000. Added `public/.gitkeep` (Next convention; Dockerfile also `mkdir -p public` for resilience). **Edge** `nginx.conf`: proxies `/api/` -> backend:8000 (full `/api/v1/...` URI preserved) and `/` -> frontend:3000 (websocket Upgrade headers set). **Orchestration** `docker-compose.yml`: backend (SQLite volume by default; commented Postgres `db` service + `PHARMACY_DB_URL` switch documented for B6) + frontend + nginx published on :8080. **CI** `.github/workflows/ci.yml`: `backend` (pytest + `mypy --strict`), `frontend` (`tsc --noEmit` + `vitest run` + `next build`), `docker-build` (builds both images). Existing `tests.yml` (legacy `archive/` suite) left untouched. `.dockerignore` files added for both contexts. **Verification:** pyproject build-system validated via `pip install -e . --dry-run`; Frontend image build pins `NEXT_PUBLIC_API_BASE_URL=http://localhost:8080` (build arg in `Dockerfile` + compose) so the bundled UI calls the nginx edge, not the unpublished `:8000`. `nginx.conf` adds `location = /health` passthrough; `docker-compose.yml` adds per-service `healthcheck` + `depends_on: service_healthy`; `docs/DEPLOYMENT.md` documents the stack and env vars. **Opt-in prod overlay** (`docker-compose.prod.yml` + `nginx.tls.conf`, activated with `-f`) enables PostgreSQL + Nginx TLS without touching the default stack. **CI** adds a `stack-smoke` job that brings the stack up and asserts `/health` + `/` return 200 through nginx. Verification: YAML/compose/CI parse + live boot enforced by CI (Docker not installed locally to run a manual `docker build`). |

| B8 — coverage >=90% + Playwright E2E | **DONE** | **Backend coverage:** added `tests/test_database_coverage.py` exercising the file-backed read replica, `VACUUM INTO` snapshot, non-SQLite engine branch (stubbed `asyncpg`), `create_schema()` PRAGMA migrate path, lazy-init of `get_session`/`get_read_session`, and the app `lifespan` startup/shutdown (`app/main.py`). Added `pytest-cov` (dev dep) + `[tool.coverage]` config; `pyproject` `addopts` now runs `--cov=app --cov-report=term-missing` with `fail_under = 90`. **Result: 90.93% coverage, 119 passed** (was 89% / 112). **Playwright E2E:** `playwright.config.ts` (boots `next start` on :3000 via `webServer`) + `e2e/pos-smoke.spec.ts` (unauthenticated `/pos` -> `/login` redirect + page renders; backend-independent smoke). Added `@playwright/test` dev dep + `test:e2e` script; `tsconfig` excludes `e2e`/`playwright.config.ts` so the Next build/tsc stay decoupled; `vitest` include is `*.test.ts` so it never grabs `*.spec.ts`. CI gained an `e2e` job (build + `playwright install --with-deps chromium` + `playwright test`). **Gates:** backend 119 passed / 90.93% / mypy 0; frontend tsc 0, vitest 23, next build OK. |
| B2 — backend security hardening | **DONE** | (a) **RBAC edges:** `tests/test_b2_security.py` asserts 403 without, 2xx with the required permission for `POST /pos/refund` (`pos.checkout`), `GET /audit/verify` + `GET /audit/export` (`inventory.read`), `GET /inventory/medicines` (`inventory.read`), `POST /shift/open` (`pos.drawer`), and `POST /auth/rotate-pepper` (`pos.pepper.rotate`). (b) **Audit export:** `AuditRepository.export_logs` + `GET /api/v1/audit/export` (json default; `?fmt=csv` → `text/csv`), gated by `inventory.read`. (c) **PIN pepper rotation:** `User.pin_pepper_version` column + v6 migration (`SCHEMA_VERSION=6`); `security.get_pin_peppers()`/`verify_pin_multi()` (rotation-safe, returns matching pepper index) + `rotate_pin_pepper()` (persists previous pepper to `pepper_path.prev`, bumps `settings.pin_pepper_version`, fail-closed on DPAPI failure); `AuthService.pin_login`/`approve_action` lazily re-hash the PIN to the current pepper when the user's version lags; `POST /auth/rotate-pepper` (perm `pos.pepper.rotate`) flags all users via `UserRepository.mark_all_pins_for_rehash`. Tests: `tests/test_b2_security.py` (RBAC + export + rotation unit/integration). Backend **128 passed / 91.07% / mypy 0** (33 files). |
| B4 — OTA delta updater tests | **DONE** | `app/services/ota_service.py` `OtaApplier`/`verify_and_apply_ota` (SHA-256-manifested, fail-closed verify→backup→copy→post-verify→rollback) now covered by `tests/test_ota.py`: happy path, invalid/missing manifest, missing source file, hash mismatch blocks apply (target untouched), post-verify failure rollback, and `verify_update` mismatch detection. Backend suite **128 passed / 91.07% / mypy 0**. |
| C1 — desktop `archive/` backlog | **DONE** (no code change) | Per user decision, `archive/` is left as-is; the B3 consolidation (zero active imports, generated dirs pruned) remains the authoritative state. No further code change. |
| C2 — frontend localization (i18n) | **DONE** | Added internationalization to the Next.js frontend: `lib/i18n/config.ts` (locales `en/de/es/fr/pt/ar`, `defaultLocale`, `localeNames`, `isLocale`), `lib/i18n/locales/{en,de,es,fr,pt,ar}.json` (curated UI-string contract), `lib/i18n/dictionaries.ts` loader, `components/I18nProvider.tsx` (`useI18n()` + `t(key)` with key fallback + localStorage persistence + `<html lang>` sync), `components/LanguageSwitcher.tsx` (endonym dropdown). `app/layout.tsx` wraps the app in `I18nProvider` and renders a header with the switcher; `app/login/page.tsx` renders its labels via `t()`. **Zero new npm dependencies** (dependency-free, to avoid offline-install/regression risk). Guard test `lib/i18n/i18n.test.ts` asserts every locale shares the default locale's key set. Frontend gates green: `tsc --noEmit` 0, `vitest` 26 (was 23), `next build` 12/12. |
| C3 — backend↔frontend contract sync | **DONE** | Verified + closed the type-contract parity gap. Audited every backend Pydantic schema in `backend_fastapi/app/shared/schemas.py` against `types/contracts.ts`; added the missing frontend interfaces `PinLoginRequest`, `UserCreate`, `MedicineCreate`, `SupplierCreate`, `AuditLogRead`, `AuditVerifyResult`, plus explicit aliases `MedicineRead`/`BatchRead`/`StockLevelRead` (already represented as `Medicine`/`Batch`/`StockLevel`). Added a dependency-free parity guard `scripts/check-contracts.mjs` (allow-lists abstract bases `ProductBase`/`MedicineBase`/`SupplierBase` + JWT-internal `TokenPayload`) and a `contract-check` CI job + `npm run check:contracts`. Run `node scripts/check-contracts.mjs` → "✓ Contract parity OK". |

**Deferred:** none — all Phase A / B / C items complete.
...
- **Phase 3–7 (OCR, Excel, Analytics, Vendors, Integrations) — DONE (2026-09-04):** Verified complete via full test suite (675 passed, 1 skipped), `mypy app --strict` 0 errors, `tsc --noEmit` 0 errors, `vitest` 33/33 pass. All Phase 3–7 items marked `[x]` in `task.md`. Note: coverage gate reports 88.45% (below `fail_under = 90`) due to newly-added route/service files lacking proportional integration test coverage — functional tests pass; coverage gap tracked as follow-up.
...
- **Tauri release readiness — DONE (2026-08-21):** `tauri.conf.json` now uses `identifier: "com.pharmacy.suite"`, `version: "1.0.0"`, a restrictive `security.csp`, `frontendDist: "../.next/static"`, `bundle.resources` mapping the Next **standalone** server into `resources/standalone`, `externalBin: ["binaries/node"]` (bundled `node.exe` 87 MB sidecar for both `x86_64-pc-windows-msvc` + `x86_64-pc-windows-gnu`), and the main window `url: "http://127.0.0.1:3000"`. The `force-dynamic` export approach was abandoned: the frontend ships 6 Next Route Handlers (`app/api/{health,currency,webhooks/paddle,auth/login,auth/refresh,auth/logout}`) implementing an httpOnly-cookie BFF, so a static `output:"export"` cannot work — the standalone server runs as a Tauri **sidecar** instead. `lib.rs` registers `tauri-plugin-shell` and spawns the node sidecar (`server.js` on `:3000`, robust path resolution over resource-dir candidates) plus a best-effort `python -m uvicorn app.main:app` backend on `:8000`. Capabilities grant `shell:allow-execute/spawn/stdin-write`. Windows Authenticode code-signing is now **CONFIGURED** (see Code Signing section below); a certificate is still required at build time, but `tauri build` works unsigned via the graceful-skip path.
...
- **Offline API docs (DONE 2026-08-22):** `/docs` and `/redoc` now serve locally-vendored Swagger UI (`swagger-ui-bundle.js` + `swagger-ui.css` + `favicon-32x32.png`) and ReDoc (`redoc.standalone.js`) assets from `backend_fastapi/app/static/`, mounted via `StaticFiles` at `/static` in `app/main.py`. The `docs_url`/`redoc_url` constructor params are set to `None` to suppress FastAPI's default CDN-backed routes; custom route handlers pass the local asset URLs. The `security_headers` middleware relaxes the CSP to `default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline'; img-src 'self' data:; font-src 'self'; connect-src 'self'` exclusively for paths matching `/docs`, `/redoc`, `/static`, `/openapi.json`; all other routes retain the strict `default-src 'none'` policy. Verified: `mypy app --strict` 0, 229 passed / 1 skipped, `/docs` renders Swagger UI with no external CDN calls.
- **v1.0.0 Release Checklist verification — DONE (2026-08-21):**
  - **Versions unified to 1.0.0:** bumped `src-tauri/Cargo.toml`, `backend_fastapi/pyproject.toml`, and `backend_fastapi/app/main.py` (`FastAPI(version="1.0.0")`); `tauri.conf.json` already `1.0.0` + `com.pharmacy.suite`.
  - **Env templates:** added root `.env.example` (frontend `NEXT_PUBLIC_*` vars, documented as build-time inlined) and `backend/.env.example` (legacy Flask license server `LEMON_SQUEEZEY_SIGNATURE_SECRET` / `ADMIN_SECRET`); `backend_fastapi/.env.example` already Creem-only.
  - **SQLite runtime check PASSED:** `pharmacy.db` WAL fully checkpointed (not locked), `user_version=6` (current schema), all data tables present (`receipt_items` singular, `products`, `sold_items`, `receipts`, `receiving_log`, `refunds`, `users`, `audit_logs`, `sync_outbox`, `shifts`, etc.).
  - **Code quality cleanup:** removed unused imports in `sync_route.py`; wrapped `lock.release()` in `posStore.ts` `try/catch` (best-effort).
  - **Git LFS attempt — REVERTED:** `.gitattributes` + node sidecar as LFS pointers were committed locally, but GitHub's LFS object storage (`github-cloud.s3.amazonaws.com`) is unreachable from this sandbox (S3 `PUT` of an 83 MB object times out) while SSH/HTTPS-to-github.com works. Per decision, the LFS commit was reverted (`git reset --hard` to `e29c241`); the node sidecar binaries remain regular git blobs (already pushed). Re-apply LFS from a machine with S3 egress if desired.
...
### Milestone M96 (Desktop Build — Tauri v2 Installer) — DONE (2026-08-21)
- **Clean production desktop build achieved.** `npx @tauri-apps/cli build` (Tauri 2.11.x, Rust 1.93.1, Node 22.23.1) produced both installers unsigned:
  - `src-tauri/target/release/bundle/msi/PharmacySuite_1.0.0_x64_en-US.msi` (42.3 MB)
  - `src-tauri/target/release/bundle/nsis/PharmacySuite_1.0.0_x64-setup.exe` (27.8 MB)
- **Fixes applied this session:** backend security (`config.py` placeholder-SECRET guard in prod, `webhook_route.py` 500→503, `seed_service.py` env-overridable admin creds, `pos_route.py` `/approve` rate-limit via `get_pin_limit()`); frontend 404 (`app/portal/page.tsx` created; `app/page.tsx` `force-dynamic` removed); `next.config.ts` `output:"standalone"` + `scripts/prepare-standalone.mjs` (copies `.next/static`/`public` next to `server.js`); `package.json` `@tauri-apps/cli`+`cross-env` + `tauri` script; `tauri.conf.json` prod identity/CSP/window-url/resource-sidecar wiring; `Cargo.toml` `tauri-plugin-shell`; `capabilities/default.json` shell perms; `src-tauri/binaries/node-{msvc,gnu}.exe` (87 MB each).
- **Backend test suite:** 221 passed, 1 skipped, 91% coverage (`fail_under=90`). `mypy app` clean.
- **Known caveats (post-tag):** (1) `location is not defined` is a non-fatal Next 16.2.10 framework warning (not our code) — harmless for the running kiosk. (2) The bundled desktop app launches the Next sidecar on `:3000` but the **FastAPI backend must also be running on `:8000`** for login/data to function; for production, run it as a service (the existing `install.ps1` NSSM `PharmacyBackend`, or spawn via the Rust sidecar which requires `python` + the backend package on the client). (3) Installers are UNSIGNED — distribute/authenticate only after applying a code-signing certificate; `npm audit` 3 high advisories in `next` remain open.

### Code Signing (Windows) — CONFIGURED (2026-08-21)

- **`bundle.windows.signCommand` REMOVED (2026-08-22):** `tauri.conf.json` no longer references `sign.cmd` — the `bundle.windows` object was removed entirely (it only contained `signCommand`). `tauri build` now produces unsigned installers without invoking any signing command. The `sign.cmd` script at `src-tauri/sign.cmd` is preserved but inactive.
- *(Historical)* `src-tauri/sign.cmd` — reads the cert from environment variables only (never committed): `SIGN_CERT_PATH` (.pfx/.p12 path), `SIGN_CERT_PASSWORD`, optional `SIGN_TIMESTAMP_URL` (default `http://timestamp.digicert.com`), and `TAURI_WINDOWS_SIGNTOOL_PATH` (optional explicit `signtool.exe`). `signtool` is auto-located via `where signtool` or the Windows SDK `bin\x64` path.
- **Graceful skip:** with no cert env set, `sign.cmd` exits 0 WITHOUT signing, so `tauri build` still yields unsigned installers (local dev). `tauri build --no-sign` also bypasses signing entirely (verified to produce both bundles in this sandbox).
- **To produce a signed release:** (1) install the Windows SDK (`signtool.exe`) + acquire an Authenticode code-signing certificate; (2) export it to a `.pfx` and set `SIGN_CERT_PATH` + `SIGN_CERT_PASSWORD` in the shell/CI; (3) `npx @tauri-apps/cli build` → `PharmacySuite_1.0.0_x64-setup.exe` / `.msi` are Authenticode-signed + RFC3161 timestamped.
- **Env limitation note:** the integrated sign step spawns `cmd.exe`, which is blocked in the CI sandbox used for this build, so the signed-build path is validated by script logic only (run on the target Windows build machine). The Node sidecar binary is already Authenticode-signed by OpenJS (official Node.js build), so it is skipped during signing.

### Milestone M97 (Clinical / Patient Management Layer — Frontend) — DONE (2026-08-25)
Backend was verified green before this phase (327 passed, 1 skipped, 90.33% coverage, `mypy --strict` 0 errors). This milestone adds the frontend clinical patient layer on top of the existing FastAPI backend (`backend_fastapi/`):

- **F1 — Contracts:** Added `PatientBase/Read/Update`, `InsurancePlanBase/Read`, `MembersGroupBase/Read/Update`, `SigCodeBase/Read`, `PriceCodeBase/Read`, `SigCodeParseResult`, `InsuranceValidationResult`, `InsuranceBindRequest`, `InsuranceValidateRequest`, `DispenseItemRead`, `DispenseBase/Create/Read`, `PatientHistoryEntry`, `BackupResult` types to `types/contracts.ts`. Extended existing `CheckoutRequest` with `client_tx_id`.
- **F2 — API services:** Created `lib/api/patients.ts` (list/search/get/create/update/delete/bind-insurance/members/dispenses/history), `lib/api/insurance.ts` (plans CRUD + validate), `lib/api/dictionaries.ts` (sig-codes CRUD + parse, price-codes CRUD, NDC lookup), `lib/api/dispense.ts` (POST dispense, GET dispense, GET label as blob).
- **F3 — Hook + Store:** Created `stores/patientStore.ts` (zustand, CRUD + search + pagination) and `hooks/usePatients.ts` (thin hook mirroring `useInventory` pattern, 300ms debounced search, RBAC via `useAuthStore.hasPermission`).
- **F4 — Patients page:** Created `app/patients/page.tsx` with 7-tab navigation (General, Insurance Plan, Members Group, Rx/Refill, Patient History, Billing Info, Comments). Co-located sub-components (no micro-files). Patient list with search, inline detail editor, insurance plan validation/binding, dependents management, dispense history table, receipt-based history with dispense IDs, and free-text comments.
- **F5 — POS dispense integration:** Extended `app/pos/page.tsx` with patient-link selector (debounced search → dropdown), "Dispense / Rx" toggle, SIG-code chip selector (calls `parseSigCode` on scan), NDC fallback inline "Add to Inventory?" action (Concern A), and `handleDispenseCheckout()` with stable `client_tx_id` via `crypto.randomUUID()` (LAN idempotency, #11). RBAC-gated via `useCan("dispense.create")`.
