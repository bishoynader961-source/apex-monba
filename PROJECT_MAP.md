...
- History backup (same release): master-history-cb28dc7.bundle as 22 MiB × 23 assets (bundle.part00–part22) — contains 7676e0f..cb28dc7 (master tip + tag v1.0.0-store-candidate), base ref 7676e0f. SHA256 a454f28c420de678c17a15d2529d772ec3bcaa02441f15f007897fa6bcf205e1; reassembly + `git bundle verify` + hash round-trip verified. Restore: `cat bundle.part* > bundle.bundle` then `git fetch <bundle> 'refs/heads/*:refs/heads/bundle-master' 'refs/tags/*:refs/tags/bundle-*'`. Whole history now recoverable off-machine even if the push never completes here.
- Residual: leftover merge-conflict marker line removed from this file (was committed in 726e44b).

## [PUSH DIAGNOSIS — 2026-09-26] RESOLVED — history rewritten, push SUCCEEDED (attempt 19)
- Attempts 17–18 (`git push --no-thin origin master`, plain + `--progress`): FAILED identically — pack generated, but output log byte-empty even with --progress (transport dies before "Enumerating objects" is emitted); afterward link too degraded for `git ls-remote` (`fatal: Could not read from remote repository`). Closure measured locally: 3,816 objects / 1,590.62 MiB raw over 31 commits (7676e0f..698a87b); tauri-build-target/ blobs ≈1.33 GB of it; excluding that dir: 962 blobs / 262.49 MiB (~232 MB = tracked src-tauri sidecar exes + 21.65 MiB app.exe).
- RESOLUTION (2026-09-26 15:20Z): git-filter-repo (in backend_fastapi/.venv) rewrote master ONLY (--refs master --partial): stripped tauri-build-target/ (1,327.60 MiB, 2,019 blobs, local-only history — 2082ad6 was NOT an ancestor of 7676e0f, so result is a pure fast-forward, NO force-push). All 22 local commits re-SHA'd (H6 dfe2bbe→9f8d6ea; commit-map preserved at .git/filter-repo/commit-map). Closure collapsed 1,513.22→185.60 MiB raw. Push attempt 19 via HTTPS+gh credential helper (SSH deploy key was deleted): SUCCESS in ~2 min at 17.5 MiB/s (pack 80.16 MiB, 921 objects), 7676e0f→9f8d6ea. Remote verified: tip 9f8d6ea, size 646.7 MiB, Phase 1-3+H6 commits visible via GitHub API. GH001 large-file warning only (node sidecar 82.95 MB). origin URL switched to https://github.com/bishoynader961-source/apex-monba.git.
- Backups kept until user deletes: full pre-rewrite bundle /tmp/apexmonba-full-backup-pre-rewrite.bundle (623,780,988 bytes, sha256 prefix 2221d0c754f9ec91 — /tmp is volatile, copy if wanted), incremental bundle pharmacysuite-incremental-cb28dc7-698a87b.bundle (52,768 bytes, sha256 45149f9e…) — the incremental is now REDUNDANT (remote holds all 5 commits) but retained per owner instruction. OLD pre-rewrite commit SHAs live on only in these bundles + the full repo.
- Every transport tried and characterized: HTTP/2 + HTTP/1.1, Content-Length vs chunked, deep `push.negotiate` (silently ignored — protocol v0/v1), multi-ref negotiation push (client still packs the full 485.43 MiB / 3315-object closure), SSH with a passphraseless deploy key. Short uploads (<=27 MiB) fly at 70-350 MiB/s; sustained uploads throttle to 14-23 KiB/s regardless of protocol (git.exe, gh, curl all identical). Attempts 7/9/11 completed the full 485 MiB upload and died at the server response phase (curl 55). This is a network-corridor property, not a git or GitHub limit — finish from a normal terminal on a healthy link.
- Server-side seeding DONE: 67 scratch branches (seed-00..63, seed2-00, seed3-00, seed-pilot) now hold 100% of the missing blob bytes (1,497 MB raw) as git objects on GitHub. They exist to enable the pack-collapse trick below and are harmless.
- Retry shortcut when the link is healthy: push master TOGETHER with the seeds in one command so their remote tips become negotiation anchors and the pack collapses to commits+trees: `git push origin master $(for n in $(seq -w 0 63); do echo "refs/remotes/origin/$n:refs/heads/seed-$n"; done) refs/remotes/origin/w2-00:refs/heads/seed2-00 refs/remotes/origin/w3-00:refs/heads/seed3-00` (the refs/remotes/origin/NN mirrors exist locally). Or just `git push origin master` on a good connection. Afterward delete scratch refs: `git push origin --delete seed-00 ... seed-63 seed2-00 seed3-00 seed-pilot` (or leave them; harmless).
- Deploy-key lifecycle completed: key created (id 164482795), used for SSH diagnosis, deleted — no credentials left behind; all temp files (keys, logs, seed scripts) removed; no orphan git/ssh processes.
- History + installer remain recoverable off-machine via release msi-v1.0.0-f884983 (SHA256-verified bundle + MSI split assets).

## [EXPANSION BLUEPRINT — PHASES 1-2 — 2026-09-26] status
- Phase 0 audit: 3 blueprint errors all FIXED at HEAD; findings C1-C4 (gift-card floats, mobile AsyncStorage tokens, CSP unsafe-inline, missing privacy policy) + H1-H6 + M1-M6 recorded; AUDIT_REPORT deferred to Phase 5.
- Phase 1 (Critical fixes, uncommitted): gift-card money -> decimal strings end-to-end (page, api layer, 2 components, backend Decimal schemas + positive-amount checks); mobile tokens -> expo-secure-store 57.0.4 (ph_access_token/ph_refresh_token; AsyncStorage = non-sensitive config only); CSP script-src 'unsafe-inline' removed ('wasm-unsafe-eval' kept — Next.js runtime needs it; full hardening deferred, noted in audit); app/privacy/page.tsx created + linked from Support. Gates: tsc 0 errors; pytest 793/3skip (fix-code e2e 39/39 when run from backend_fastapi/ CWD — config.py:126 loads .env relative to CWD, pre-existing quirk).
- Phase 2 (recovery system, uncommitted): crash_handler.rs (panic hook first in run(), sanitized 200-char crash JSON to %APPDATA%/PharmacySuite/crashes, native MessageBox); lib.rs HTTP health polling (poll_health) + port-conflict pre-checks :3000/:8000 (closes H4) + recovery window on failure (codes FRONTEND/BACKEND_UNHEALTHY); recovery.html + recovery-app.js compiled in via include_str!, served on recovery:// scheme (CSP allows scheme), capability recovery.json + permissions/recovery.toml (manual TOML convention — auto-gen only covers lib.rs commands, so command wrappers live in lib.rs delegating to fix_engine); fix_engine.rs = Python-canonical-json HMAC-SHA256 (sha2/hmac crates, constant-time compare), hardcoded FIX_CODE_KEY (include_str! fix_key.txt = backend .env value, per owner decision 2026-09-26) + admin key fix_admin_key.txt (shown once: [REDACTED-ROTATED-2026-09-26] — store in password manager), whitelist: update_setting (5 keys)/delete_crash_logs/reset_config (.env + window-state.json only, NEVER pharmacy.db/secret.key); offline-recovery.html (static, script-free) deployed by NSIS_HOOK_POSTINSTALL to $LOCALAPPDATA/PharmacySuite/recovery/; pharmacysuite-recovery.exe = second cargo bin (no Tauri/WebView2), subcommands status/copy-diag/apply-fix/launch, smoke-tested. Step 2.7: mailto body control-char guard on Support page.
- Gates after Phase 2: tsc 0 errors; cargo check 0 errors (6 pre-existing warnings); recovery exe runtime smoke PASS.
- Key rotation runbook: FIX_CODE_SECRET lives in TWO places (backend .env + src-tauri/src/fix_key.txt) — change both atomically (KNOWN_ISSUES #11).

## [EXPANSION BLUEPRINT — PHASE 3 — 2026-09-26] Mobile Diagnostic & Recovery System — COMMITTED
- Adapted all blueprint `mobile/src/**` paths to the REAL layout (mobile is React Navigation, NOT Expo Router: App.tsx + navigation/RootNavigator.tsx + screens/*.tsx + lib/api/*).
- `mobile/lib/diagnostics/classifier.ts` — classifyError/safeStringify (cycle-safe), writeCrashLog ring buffer (10, AsyncStorage `ph_crash_log`), ERR_* codes + ERROR_MESSAGES map.
- `mobile/components/ErrorBoundary.tsx` wraps `<RootNavigator />` in App.tsx (app-level desktop boundary unchanged). `mobile/lib/api/health.ts` — probeHealth (5s timeout), healthy/degraded(≥1 fail)/offline(≥3) states, onHealthChange listeners; offline guard never clears session. Health-check deviation from blueprint (expo-background-fetch REJECTED → in-process setInterval, foreground-only, iOS throttling rationale) documented as 4-point comment at startHealthPolling.
- `mobile/lib/security/fixEngine.ts` (GITIGNORED — embeds real secret; commit only `fixEngine.ts.template` placeholder, done in 7d9dec2): expo-crypto RFC-2104 HMAC-SHA256, Python json.dumps(sort_keys=True) canonical JSON, constant-time compare, same whitelist ops as desktop engine (mobile: clear_local_cache/reset_connection_settings/update_mobile_setting/force_full_sync). Secret now lives in THREE places — rotation must be atomic across backend .env + fix_key.txt + fixEngine.ts (supersedes two-place runbook above).
- `mobile/screens/MobileRecoveryScreen.tsx` (RecoveryActions: auto-fix, Share-API diagnostics, fix-code input, probe, support notice) + SettingsScreen Support & Recovery section with red badge; `lib/api/client.ts` axios module augmentation for error.status.
- SECURITY VERIFICATION 2026-09-26 (all clean pre-commit): 1a key files never in any of 228 reachable commits (pathspec incl. src-tauri/src/fix_admin_key.txt); 1b all four paths ignored (bare-name fix_key.txt/fix_admin_key.txt guards ADDED to .gitignore this pass — any location protected); 1c status tracked-clean, only .example/.template tracked; 1d FIX_CODE_SECRET/fix_key appear as NAMES only (signing-CLI lineage, .gitignore, template — by design), zero hardcoded secret assignments in 1d diff dumps, zero long-hex except commit SHAs + MSI/bundle SHA256 doc values. VALUE sweeps all ZERO hits: live FIX_CODE_SECRET (62 chars, from backend .env), revoked admin key e2a54f66…, current admin key (read from gitignored file), prefixes 7f3d9a/e2a54f66; git fsck --unreachable EMPTY (purge complete). Remote master = 7676e0f, remote has NO mobile/ or fix-key paths (GitHub MCP 404 probes) — nothing ever reached GitHub.
- SECURITY INCIDENT (disclosed): commit b1457f5 (superseded Phase-2 commit) accidentally included the OLD admin key VALUE in PROJECT_MAP.md. Remediation: key ROTATED (new key in gitignored src-tauri/src/fix_admin_key.txt, shown once in session transcript), old value scrubbed, history rebuilt (reset --soft to cfa7a92, re-committed), reflog expired + gc pruned; value/prefix sweeps above prove zero residual. Old key rendered useless by rotation regardless.
- Phase-3 commit files: .gitignore, mobile/App.tsx, mobile/lib/api/client.ts, mobile/package{,-lock}.json, mobile/screens/{SettingsScreen,MobileRecoveryScreen}.tsx, mobile/components/ErrorBoundary.tsx, mobile/lib/api/health.ts, mobile/lib/diagnostics/classifier.ts, PROJECT_MAP.md. NOT committed (gitignored): mobile/lib/security/fixEngine.ts; AGENT_TRIGGER_PROMPT.md + PHARMACY_SUITE_EXPANSION_BLUEPRINT.md remain untracked (owner's call). Gate: mobile tsc --noEmit exit 0. Phase 4 NOT started — awaiting owner authorization.
- H6 CLOSED (2026-09-26, pre-Phase-4): WC claim money validation — WCClaimBase/WCClaimUpdate money fields now Field(ge=0, le=99999999.99) matching Numeric(10,2) columns; wc_route `_validate_money_totals` enforces total_charges == insurance_paid + patient_responsibility on create AND update (mirrors Phase-1 checkout contract; unadjudicated zero-default claims allowed; updates require balanced triplet or none). 8 new tests in test_wc_claims.py; backend suite 803 passed / 1 skipped / 0 failed (coverage-gate FAIL line is pre-existing pyproject addopts state, unrelated).

## [EXPANSION BLUEPRINT — PHASE 4 — 2026-09-26] Task 1 — Steps 1.1–1.4 (STOP after 1.4 per owner instruction)
- Step 1.1 parity audit vs blueprint 1.2.1 (real layout, NOT mobile/src/**): P0 auth ✓, POS ✓, RX queue ✓, Inventory ✓ (+adjust via /inventory/adjust), Patient Search ✗ → P1 (deferred, owner review); P1 expiry alerts ✗ (backend /alerts exists), analytics ✓, WC claims ✗, settings ✓, drug interactions ✗ (backend /drug-interactions/check exists); P2 audit log ✗ (backend /audit/verify+export); Label Engine + Bulk Import deliberately desktop-only.
- Step 1.2 DONE (51bb566): mobile contracts was a stale 511-line subset (desktop 2151) with 82 mobile-only lines — merged to ONE 219-declaration source written byte-identical to mobile/types/contracts.ts + types/contracts.ts (LF-normalized); SyncPushEntry/SyncPushResult kept as mobile supersets (offline-queue fields), all collisions otherwise resolved to backend-synced desktop; decimalCurrency.ts = desktop superset copied over. Fixed latent bug: AnalyticsScreen velocity color map keyed FAST/MODERATE/SLOW vs backend FAST_MOVING/… (badges always gray). CI contract-check job now cmp's both pairs. Gates: desktop+mobile tsc 0 errors.
- Step 1.3 DONE: authStore tokens moved AsyncStorage→SecureStore (ph_access_token/ph_refresh_token) with one-time legacy migration+scrub (pre-1.3 builds wrote keys the interceptor never read — live auth bug fixed); client.ts exports setUnauthorizedCallback/setForbiddenCallback/setActivityCallback + getStoredAccessToken/storeTokens/clearStoredTokens; interceptor fires 403 → alert (never crash), success → idle-clock activity; authStore session machine = idle+absolute timeouts from SystemSetting via new lib/api/settings.ts (defaults 15/480 mirroring desktop sessionStore + backend seeds), 30s watcher in App.tsx, logout nulls session FIRST (no 401↔logout loop); biometric unlock (expo-local-authentication ~57.0.3) = foreground re-validation of an EXISTING SecureStore session only — toggle in Settings > Security, unlock button on LoginScreen when enabled. Gate: mobile tsc 0 errors.
- Step 1.4 DONE (DATABASE MODE): seed_service.seed_default_settings + mobile_access_mode (default "shared") + qr_secret_key (empty placeholder for Step 1.5 signed-QR; reseeding never clobbers admin values — 3 tests in test_mobile_access_mode_seeds.py); app/shared/config.py Settings + mobile_access_mode/mobile_bind_host (MOBILE_ACCESS_MODE/MOBILE_BIND_HOST); src-tauri spawn_servers now reads mobile_access_mode via new read_system_setting_value() helper (generalized from read_terminal_mode, parameterized + tuple-bind like update_system_setting) and passes --host 0.0.0.0 for "shared", 127.0.0.1 for "independent"/cloud-stub/unknown (fail-safe); run_backend.py --host flag already plumbed (Tauri CLI overrides its 0.0.0.0 default). Desktop Settings gained Mobile Access (Network) section (Smartphone icon, mode select + firewall note + restart notice, saveSection convention). Gates: seed tests 3/3, mobile tsc 0, desktop tsc 0, cargo check 0 errors (same 6 pre-existing warnings). HARD STOP reached — Steps 1.5 (QR tab + device_route + Device model) and 1.6 (mDNS) NOT started, awaiting owner review per instruction.
