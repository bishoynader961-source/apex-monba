          ...
          {!selectedId && !isEditing ? (
            ...
            <div className="grid grid-cols-2 gap-x-8 gap-y-5">
              ...
              <div className="space-y-1">
                <label className="text-xs text-gray-400 font-medium uppercase tracking-wider">Drug Name</label>
                <input disabled={!isEditing} type="text" value={formData.name || ""} onChange={e => updateField("name", e.target.value)}
              ...
              <div className="space-y-1">
                <label className="text-xs text-gray-400 font-medium uppercase tracking-wider">NDC Code (11-digit)</label>
                <div className="flex gap-2">
                  <input disabled={!isEditing} type="text" value={formData.ndc_code || ""} onChange={e => updateField("ndc_code", e.target.value)}
              ...
              <div className="space-y-1">
                <label className="text-xs text-gray-400 font-medium uppercase tracking-wider">Strength</label>
                <input disabled={!isEditing} type="text" value={formData.strength || ""} onChange={e => updateField("strength", e.target.value)}
              ...
              <div className="space-y-1">
                <label className="text-xs text-gray-400 font-medium uppercase tracking-wider">Dosage Form</label>
                <select disabled={!isEditing} value={formData.form || ""} onChange={e => updateField("form", e.target.value || null)}
                  className="w-full bg-black/40 border border-white/10 rounded-md p-2.5 text-sm focus:border-green-500 outline-none disabled:opacity-50">
                  <option value="">—</option>
              ...
              <div className="space-y-1">
                <label className="text-xs text-gray-400 font-medium uppercase tracking-wider">DEA Schedule</label>
                <select disabled={!isEditing} value={formData.dea_schedule || ""} onChange={e => updateField("dea_schedule", e.target.value || null)}
                  className="w-full bg-black/40 border border-white/10 rounded-md p-2.5 text-sm focus:border-green-500 outline-none disabled:opacity-50">
                  <option value="">—</option>
              ...
              <div className="space-y-1">
                <label className="text-xs text-gray-400 font-medium uppercase tracking-wider">Therapeutic Class</label>
                <input disabled={!isEditing} type="text" value={formData.therapeutic_class || ""} onChange={e => updateField("therapeutic_class", e.target.value || null)}
                  className="w-full bg-black/40 border border-white/10 rounded-md p-2.5 text-sm focus:border-green-500 outline-none disabled:opacity-50" />
              </div>

              <div className="space-y-1">
                <label className="text-xs text-gray-400 font-medium uppercase tracking-wider">Manufacturer</label>
                <input disabled={!isEditing} type="text" value={formData.manufacturer_name || ""} onChange={e => updateField("manufacturer_name", e.target.value || null)}
                  className="w-full bg-black/40 border border-white/10 rounded-md p-2.5 text-sm focus:border-green-500 outline-none disabled:opacity-50" />
              </div>

              {pillImageUrl && (
                <div className="col-span-2 space-y-1">
                  <label className="text-xs text-gray-400 font-medium uppercase tracking-wider">Pill Image (from API)</label>
              ...
              <div className="space-y-1">
                ...
                <input disabled={!isEditing} type="text" value={formData.category || ""} onChange={e => updateField("category", e.target.value || null)}
                  className="w-full bg-black/40 border border-white/10 rounded-md p-2.5 text-sm focus:border-green-500 outline-none disabled:opacity-50" />
              </div>
              ...
              <div className="space-y-1">
                <label className="text-xs text-gray-400 font-medium uppercase tracking-wider">Retail Price</label>
                <input disabled={!isEditing} type="number" step="0.01" value={formData.price || "0.00"} onChange={e => updateField("price", e.target.value)}
              ...
              <div className="space-y-1">
                <label className="text-xs text-gray-400 font-medium uppercase tracking-wider">Wholesale Price</label>
                <input disabled={!isEditing} type="number" step="0.01" value={formData.wholesale_price || ""} onChange={e => updateField("wholesale_price", e.target.value || null)}
                  className="w-full bg-black/40 border border-white/10 rounded-md p-2.5 text-sm font-mono focus:border-green-500 outline-none disabled:opacity-50" />
              </div>
              ...
              <div className="space-y-1">
                <label className="text-xs text-gray-400 font-medium uppercase tracking-wider">Vendor</label>
                <input disabled={!isEditing} type="text" value={formData.vendor_name || ""} onChange={e => updateField("vendor_name", e.target.value)}
                  className="w-full bg-black/40 border border-white/10 rounded-md p-2.5 text-sm focus:border-green-500 outline-none disabled:opacity-50" />
              </div>

              <div className="space-y-1">
                <label className="text-xs text-gray-400 font-medium uppercase tracking-wider">Reorder Threshold</label>
                <input disabled={!isEditing} type="number" value={formData.reorder_threshold ?? ""} onChange={e => updateField("reorder_threshold", parseInt(e.target.value) || null)}
                  className="w-full bg-black/40 border border-white/10 rounded-md p-2.5 text-sm font-mono focus:border-green-500 outline-none disabled:opacity-50" />
              </div>
            </div>
          )}
