...
class TestNDCDictionary(unittest.TestCase):
    ...
    def test_normalize_dea(self):
        ...
        Note: _DEA_SCHEDULES has lowercase keys ("a","b","c","na") that never
        match because _normalize_dea uppercases input. "na" -> "NA" (not "OTC").
        Only uppercase keys (CII, CIIS, CIII, CIV, CV) and the empty-string
        guard are effective.
        """
        from ndc_dictionary import _normalize_dea

        # SCHEDULE prefix: key is stripped but II/III/IV not in _DEA_SCHEDULES,
        # so the default (raw uppercased) is returned.
        ...
        # CIIS -> CII (uppercase key maps in _DEA_SCHEDULES)
        self.assertEqual(_normalize_dea("CIIS"), "CII")

        # cIV -> CIV (uppercased to CIV, which is an uppercase key)
        self.assertEqual(_normalize_dea("cIV"), "CIV")

        # Lowercase Roman-style: cii -> CII, cIII -> CIII
        ...
        # Bug documentation: "na" should map to "OTC" per the dict,
        # but since the key is uppercased to "NA" (which is not a dict key),
        # the raw uppercased value is returned instead.
        self.assertEqual(_normalize_dea("na"), "NA")
