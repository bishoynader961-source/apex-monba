...
def setup_patients_tab(self):
    ...
    ctk.CTkButton(
        top_frame, text="Add Patient", width=120, fg_color="#28a745", hover_color="#218838",
        command=lambda: _open_patient_dialog(self),
    ).grid(row=0, column=1, padx=(0, 5))

    ctk.CTkButton(
        top_frame, text="Edit", width=80, fg_color="#e67e22", hover_color="#cf6d17",
        command=lambda: _edit_selected_patient(self),
    ).grid(row=0, column=2, padx=(0, 5))

    ctk.CTkButton(
        top_frame, text="Delete", width=80, fg_color="#dc3545", hover_color="#c82333",
    ...
    style.configure("Treeview",
                     ...
                     rowheight=25, fieldbackground="#2b2b2b",
    ...
    columns = ("Name", "Phone", "Email", "Custom Fields", "Created")
    self.tree_patients = ttk.Treeview(self.tab_patients, columns=columns, show="headings")
    self.tree_patients.heading("Name", text="Name")
    self.tree_patients.heading("Phone", text="Phone")
    self.tree_patients.heading("Email", text="Email")
    self.tree_patients.heading("Custom Fields", text="Custom Fields")
    self.tree_patients.heading("Created", text="Created")

    self.tree_patients.column("Name", width=160, anchor="w")
    self.tree_patients.column("Phone", width=120, anchor="w")
    self.tree_patients.column("Email", width=160, anchor="w")
    self.tree_patients.column("Custom Fields", width=280, anchor="w")
    self.tree_patients.column("Created", width=130, anchor="center")

    self.tree_patients.grid(row=1, column=0, sticky="nsew", padx=10, pady=(0, 10))
...
def _open_patient_dialog(self, patient_id=None):
    ...
    dialog.title("Edit Patient" if patient_id else "Add Patient")
    ...
    ctk.CTkLabel(form, text="Name:", font=ctk.CTkFont(weight="bold")).grid(row=0, column=0, sticky="w")
    name_entry = ctk.CTkEntry(form, width=400, placeholder_text="Patient full name")
    name_entry.grid(row=1, column=0, sticky="ew", pady=(0, 8))
    if patient:
        name_entry.insert(0, patient[1])

    ctk.CTkLabel(form, text="Phone:", font=ctk.CTkFont(weight="bold")).grid(row=2, column=0, sticky="w")
    phone_entry = ctk.CTkEntry(form, width=400, placeholder_text="Phone number")
    phone_entry.grid(row=3, column=0, sticky="ew", pady=(0, 8))
    if patient:
        phone_entry.insert(0, patient[2])

    ctk.CTkLabel(form, text="Email:", font=ctk.CTkFont(weight="bold")).grid(row=4, column=0, sticky="w")
    email_entry = ctk.CTkEntry(form, width=400, placeholder_text="Email address")
    ...
    def on_save():
        ...
        if not name:
            messagebox.showwarning("Required", "Patient name is required.", parent=dialog)
            return
