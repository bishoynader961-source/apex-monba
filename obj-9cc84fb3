...
# Replace the multiline className with a single-line version
# Pattern matches from "className={selectedUser?.id === u.id" to the closing "}>"
pattern = re.compile(
...
def replace_tr(match):
    return 'className={selectedUser?.id === u.id ? "border-b border-gray-800/50 bg-blue-900/20" : "border-b border-gray-800/50 hover:bg-white/5"}'

content = re.sub(pattern, '<tr key={u.id} className={selectedUser?.id === u.id ? "border-b border-gray-800/50 bg-blue-900/20" : "border-b border-gray-800/50 hover:bg-white/5"}>', content)
