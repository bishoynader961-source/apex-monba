...
# Fix line 128-130: multiline className -> single line
old_block = '''<tr key={u.id} className={selectedUser?.id === u.id 
                            ? "border-b border-gray-800/50 bg-blue-900/20" 
                            : "border-b border-gray-800/50 hover:bg-white/5"}>'''
new_block = '<tr key={u.id} className={selectedUser?.id === u.id ? "border-b border-gray-800/50 bg-blue-900/20" : "border-b border-gray-800/50 hover:bg-white/5"}>'
content = content.replace(old_block, new_block)

# Fix line 134: span className with template literal
