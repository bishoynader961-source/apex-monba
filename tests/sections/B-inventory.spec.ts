...
test.describe('Section B: Inventory Tab Buttons (requires valid license)', () => {
  ...
  test('Inventory page requires valid license', async ({ page }) => {
    ...
    if (headingText?.includes('License Validation') || headingText?.includes('License')) {
      console.log('License validation page detected - Inventory requires valid license');
      ...
      console.log('License validation page verified - Inventory requires valid license');
      return;
    }
    
    // If we get here, Inventory page loaded (valid license present)
    await expect(page.locator('table')).toBeVisible({ timeout: 10000 });
  });

  test('Inventory page license validation has expected elements', async ({ page }) => {
    await page.goto('/dashboard/inventory');
    await page.waitForLoadState('domcontentloaded', { timeout: 30000 });
    await page.waitForTimeout(3000);
